...
string getCoordinateRangeFromQuarter(int quarterNumber)
{
    ...
    if (quarterNumber == 1)
    {
        result = ($"Четверть номер {quarterNumber} находится в координатах X > 0, Y > 0!");
    }
...
int quarterNumber = getQuarterFromUser("Введите номер четверти - целое число от 1 до 4");
Console.WriteLine(getCoordinateRangeFromQuarter(quarterNumber));