...
int getSumOfNegativeNumbersInArray(int[] incomingArray)
{
    ...
    for (int i = 0; i < incomingArray.Length; i++)
    {
        if (incomingArray[i] < 0)
        {
            result += incomingArray[i]; //result = result + incomingArray[i];
        }
    }
    return result;
}
...
Console.WriteLine($"Сумма положительных чисел равна {sumOfPositive}, сумма отрицательных чисел равна {sumOfNegative}");
